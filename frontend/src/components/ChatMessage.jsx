...
function MediaCard({ kind, item }) {
  ...
  const meta =
    kind === "movie"
      ...
      : kind === "book"
        ? (item.authors || []).slice(0, 2).join(", ") || "Google Books"
        : kind === "music"
          ? (item.artists || [])
...
export default function ChatMessage({ message }) {
  ...
  const records = [];
  (result.sources || [])
    .filter((source) => source.type === "tool_result")
    .forEach((source) =>
      Object.entries({
        movies: "movie",
        books: "book",
        ...
        (source.result?.[key] || [])
          .slice(0, 6)
          .forEach((item) => records.push({ kind, item })),
      ),
    );
  return (
    <div className="message assistant">
      ...
      {records.length > 0 && (
        <>
          <div className="section-title">Similar media</div>
          <div className="media-grid">
            {records.map(({ kind, item }, index) => (
              <MediaCard key={`${kind}-${index}`} kind={kind} item={item} />
            ))}
          </div>
        </>
      )}
